...
def main(filename):
    ...
    # for part 2 we want to look for shortest path from any 0 elevation to end
    # we invert the BFS, we start from E and can only take one-sized steps down
    # we should hit the closest 0 elevation spot first
    queue = deque()
    queue.append((end, 0))
    visited = {end}
    while queue:
